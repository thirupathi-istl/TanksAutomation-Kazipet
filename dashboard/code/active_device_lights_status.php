...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (!$conn_db_all) {
		...
		$sql = "";
		if ($device_status == "ON_LIGHTS") {
			$sql = "SELECT active_device, device_id, date_time, ping_time, installed_date, installed_status, location FROM live_data_updates 
			WHERE active_device='1' AND installed_status = '1'AND ( on_off_status=1 or on_off_status=3 or on_off_status=4) AND device_id IN ($placeholders) ORDER BY LENGTH(device_id), device_id";
		} 
		elseif ($device_status == "OFF_LIGHTS") {
			$sql = "SELECT active_device, device_id, date_time, ping_time, installed_date, installed_status, location FROM live_data_updates 
			WHERE active_device='1' AND installed_status = '1'AND  on_off_status=0 AND device_id IN ($placeholders) ORDER BY LENGTH(device_id), device_id";
		} 
		elseif ($device_status == "MANUAL_ON") {
			$sql = "SELECT active_device, device_id, date_time, ping_time, installed_date, installed_status, location FROM live_data_updates 
			WHERE active_device='1' AND installed_status = '1'AND on_off_status=5 AND device_id IN ($placeholders) ORDER BY LENGTH(device_id), device_id";
		} 
            ...
            if (mysqli_num_rows($result) > 0) {
            	while ($r = mysqli_fetch_assoc($result)) {
            		...
            		if ($device_status == "ON_LIGHTS") {
            			...
            			<td><button class="btn fw-semibold  btn-info btn-sm p-0 px-2 " onclick=openOpenviewModal("'.$device_id.'") >View</button></td>
            			<td>'.$address.'</td> 
            			</tr>';
            			
            		} 
            		else if ($device_status == "OFF_LIGHTS") {
            		...
            		else if ($device_status == "MANUAL_ON") {
